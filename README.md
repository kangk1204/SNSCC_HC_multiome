...
_Local note:_ the root scripts now match the latest `new/` folder (superseding the previous upstream snapshot). If you modify or rerun the pipeline, keep this README in sync with any changes to file names or paths.
...
- Please cite the SNSCC multiome study when referencing these scripts (add DOI or publication details once available).
- Add your contact details here if sharing this folder internally.
